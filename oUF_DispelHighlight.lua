...
assert(oUF, 'oUF DispelHighlight was unable to locate oUF install')
...
local function Update(self, event, unit)
	...
	if(self.PreUpdateDispelHighlight) then
		self:PreUpdateDispelHighlight()
	end
	...
	if(self.PostUpdateDispelHighlight) then
		return self:PostUpdateDispelHighlight(debuffType, removable)
	end
end

local function Path(self, ...)
	return (self.OverrideDispelHighlight or Update)(self, ...)
end

local function Enable(self)
	if(self.DispelHighlight) then
		if(self.DispelHighlight:IsObjectType('Texture')) then
			if(self.DispelHighlight:GetTexture()) then
				self.__highlightMethod = 'SetVertexColor'
			else
				self.__highlightMethod = 'SetColorTexture'
			end

			self.__highlightTarget = self.DispelHighlight
		else
			self.__highlightMethod = 'SetBackdropColor'
			self.__highlightTarget = self
		end
	elseif(self.DispelHighlightBorder) then
